...
with open(log_file_name, "w") as f:
    ...
    for epoch in range(EPOCHS):
        ...
        for mfccs, labels, mfcc_lengths, paths, captions in train_dataloader:
            ...
            if isinstance(labels, list) and len(labels) > 0 and labels[0].dim() == 2:
                new_labels = []
                for label in labels:
                    indices = []
                    for row in label:
            ...
            if target_lengths.sum() > input_lengths.sum():
                f.writelines("警告: target_lengths 超过 input_lengths，跳过该 batch \n")
            ...
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.W_out.parameters(), max_norm=CLIP_GRAD)
            optimizer.step()
        ...
        if avg_loss < best_loss:
            ...
            print(f"最优模型已保存！Loss: {best_loss:.4f} | 保存至: {MODEL_SAVE_PATH}")
            f.writelines(
