...
transform_mfcc = MFCC(**torchless_mfcc_settings)
...
class TidigitDataset:
    def __init__(
        ...
        transformer: Any = transform_mfcc,
    ...
    def _load_data(self, extension: str):
        ...
        包括：归一化，转单声道、采样率统一、MFCC 计算、归一化以及 caption 编码
        """
        self.dataset_path_list = get_audio_abs_path_list(self.data_dir, extension)

        for audio_abs_path in self.dataset_path_list:
            try:
                # normalize the original signals to adapt to the net
                # waveform, sample_rate = torchaudio.load(audio_abs_path, normalize=True)
                waveform, sample_rate = librosa.load(audio_abs_path, sr=None)
                # if torchless_librosa_settings["ori_sig_normalize"]:
                if True:
                    # waveform = (waveform - waveform.mean()) / waveform.std()
                    librosa.util.normalize(waveform)
                elif torchless_librosa_settings["ori_sig_use_l1"]:
                    librosa.util.normalize(waveform, norm=1)
                elif torchless_librosa_settings["ori_sig_use_l2"]:
                    librosa.util.normalize(waveform, norm=2)
                ...
                if sample_rate != target_sample_rate:
                    logging.info(
                        f"the sample rate is not the target sample rate, resampling to {target_sample_rate}"
                    )
                    waveform = librosa.resample(
                        waveform, orig_sr=sample_rate, target_sr=target_sample_rate
                    )

                # 计算 MFCC，原始形状为 (1, T, n_mfcc)
                mfcc: np.ndarray = self.transformer(
                    waveform, target_sample_rate
                ).squeeze(0)  # -> (T, n_mfcc)

                # 对 MFCC 进行零均值、单位方差归一化（按每个特征维度计算均值和标准差）
                mean = np.mean(mfcc, axis=0, keepdims=True)
                std = np.std(mfcc, axis=0, keepdims=True) + 1e-6  # 防止除 0
                mfcc = (mfcc - mean) / std
                ...
                # label processing
                if self.use_onehot:
                    label = self.caption_encoder(caption, onehot=True)
                else:
                    label = self.caption_encoder.encode(caption)
                # label = (
    ...
    def __add__(self, other):
        if isinstance(other, TidigitDataset):
            raise TypeError("Only TidigitDataset can be added to TidigitDataset")
        abs_path_list: list[str] = []

        def add(x):
            if isinstance(x, list):
                abs_path_list.extend(x)
            else:
                abs_path_list.append(x)
